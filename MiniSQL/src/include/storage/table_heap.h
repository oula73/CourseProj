...
public:
   ...
   * @return true if the delete is successful (i.e the tuple exists)
   */
...
private:
  ...
  explicit TableHeap(BufferPoolManager *buffer_pool_manager, Schema *schema, Transaction *txn,
      ...
      page_id_t pageId;
      auto *tablePage = reinterpret_cast<TablePage *>(buffer_pool_manager->NewPage(pageId));
      ASSERT(tablePage != nullptr, "cannot create a new page");
      tablePage->Init(pageId, INVALID_PAGE_ID, log_manager, txn);
      first_page_id_ = pageId;
      insert_page_id_ = first_page_id_;
      buffer_pool_manager_->UnpinPage(pageId, true); // 要记得unpin
  };
